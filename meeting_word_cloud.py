...
def buscar_meeting_code_processado(meeting_code):
    ...
    records = table_service.query_entities("reactionTracking", filter="PartitionKey eq 'audio-analysis' and RowKey eq '" + meeting_code + "'")
...
def enviar_aquivos_audio_blob(main_app, dir="audio_files/"):
    for file in glob.glob(dir + "*.wav"):
        try:
            ...
            payload = {"meeting-code": meeting_code, "blob": blob, "file-name": util.get_file_with_extension(file)}
